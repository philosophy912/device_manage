  ...
  <div class="app-container">
    ...
    <el-table :key="tableKey" v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%;" @sort-change="sortChange">
      ...
      <el-table-column :label="$t('goods.image')" min-width="120px" align="center">
        <template slot-scope="{row}">
          <img v-if="row.image" :src="row.image" alt="" width="100%" height="100%"><img>
          <span v-if="!row.image">无</span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('goods.recipients_status')" min-width="70px">
        <template slot-scope="{row}">
          <span>{{ handleRecipientsStatus(row.recipientsStatus) }}</span>
        </template>
      </el-table-column>
...
<script>
import { fetchGoodsRecipientsList, createGoods, updateGoods, deleteGoods } from '@/api/goods'
